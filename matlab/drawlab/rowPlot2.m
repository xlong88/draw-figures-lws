...
%
%
% Usage : rowPlot2(X, Ys, Name,Value)
%
% Input :
%           X   --   a vector
%           Ys  --   a matrix
...
function rowPlot2(X, Ys, dataType, varargin)
    ...
    switch lower(varargin{i})
        ...
        case {'plottype'}
            if length(varargin{i + 1}) == 1
                ...
                myPlot = {@plot, @plot, @plot, @plot};
                plotType = varargin{i + 1};
                for i=1:length(length(varargin{i + 1}))
                    switch varargin{i + 1}(i)
                        case {'plot'}
                            myPlot{i} = @plot;
                        case {'loglog'}
                            myPlot{i} = @loglog;
...
% height for xlabel
xLabelh = yLabelw + 0.005;

% height for title

if exist('title_', 'var') ~= 0 && ~isempty(title_)
    titleh = 0.05;
else 
    titleh = 0.01;
end
...
for i = 1:4
    ...
    for k = 1:K
        ...
        if strcmp(dataType, 'homo')
            if iscell(X)
                mp(X{i},Ys{i}(:,k),'Parent',ax{i}, lspec{:});
            else
                mp(X,Ys{i}(:,k),'Parent',ax{i}, lspec{:});
            end
        else
            mp(X{i, k},Ys{i, k},'Parent',ax{i}, lspec{:});
        end
    end
    if exist('ylabel_','var') ~= 0
        if i == 1
            if ylabel_(1) == '$'
                ylabel(ylabel_,'FontSize', ylabelFont, 'Interpreter', 'latex')
            else 
    ...
    if exist('xlabel_', 'var') ~= 0
        if ~iscell(xlabel_) || length(xlabel_) == 1
            xll = xlabel_;
        else
            xll = xlabel_{i};
        end
        if xll(1) == '$'
            xlabel(xll, 'FontSize', xlabelFont, 'Interpreter', 'latex')
        else 
            xlabel(xll, 'FontSize', xlabelFont)
        end
    end
    ...
    if exist('title_', 'var') ~= 0 && ~isempty(title_)
        if iscell(title_)
            if title_{i}(1) == '$' 
                title(title_{i},'FontSize', titleFont, 'Interpreter', 'latex');
            else
